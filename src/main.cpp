...
//chords
struct ChordEvent {
    int keys[4];      // up to 4 notes in a chord
    int keyCount;     // how many notes are in this chord
    int duration;     // how long to hold the chord
  };
  ...
  void playChord(const ChordEvent& chord) {
    ...
    delay(50); // short rest between chords
  }
