...
// ─────── S3 STUBS ───────────────
#include "driver/i2s_std.h"    // use the new I2S API on S3
#ifndef I2S_MODE_DAC_BUILT_IN
#define I2S_MODE_DAC_BUILT_IN 0
#endif
#ifndef I2S_DAC_CHANNEL_BOTH_EN
...
void BluetoothA2DPSink::init_i2s() { 
    ...
    if (is_i2s_output) {
        ...
        // ── on ESP32 we configure the DAC pins; on S3 we skip all of this ──
...
esp_err_t BluetoothA2DPSink::i2s_mclk_pin_select(const uint8_t pin) {
    // S3 has no GPIO-based MCLK output; stub it out
